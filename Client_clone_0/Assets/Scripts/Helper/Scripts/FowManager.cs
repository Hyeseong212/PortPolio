...
namespace Rito.FogOfWar
{
    ...
    public class FowManager : MonoBehaviour
    {
        ...
        ***********************************************************************/
        ...
        public List<GameObject> Statusbar; // 모든 플레이어 오브젝트를 참조
        ...
        public IEnumerator UpdateFogRoutine()
        {
            while (true)
            {
                ...
                for (int i = 0; i < players.Count; i++)
                {
                    ...
                    // 상태 표시줄의 활성화/비활성화 제어 (플레이어가 활성화되어 있을 때만)
                    statusbar.SetActive(isPlayerVisible && player.activeSelf);
                }
                yield return new WaitForSeconds(_updateCycle);
            }
        }
        public bool IsCharacterVisible(GameObject character)
        {
            foreach (var unit in UnitList)
            {
                if (Vector3.Distance(unit.transform.position, character.transform.position) <= unit.sightRange)
                {
                    // 장애물 여부 확인
                    if (!Physics.Linecast(unit.transform.position, character.transform.position, _groundLayer))
                    {
                        return true;
                    }
                }
